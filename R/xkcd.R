...
#constructor is returning x so we don't need to specify it
new_xkcd <- function(x) {
...
validate_xkcd <- function(x) {
  ...
  if (!all(required_fields %in% names(x))) {
    difference <- setdiff(required_fields, names(x)) #what is this
    stop("xkcd object is missing the following required fields: ",
         paste(difference, collapse = ", "))
  } #if statement always returns null.
...
plot.xkcd <- function(x, ...) {
  ...
  plot.new() #what is this?
  grid::grid.raster(p) #what is this?
}
